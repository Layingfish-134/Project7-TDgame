...
class SilencerEnemy : public Enemy
{
public:
	SilencerEnemy()
	{
		ConfigManager::EnemyTemplate& tpl = ConfigManager::instance()->silencer_template;
		SDL_Texture* tex_idle = ResourcesManager::instance()->get_texture_pool().find(ResID::Tex_SilencerIdle)->second;
		SDL_Texture* tex_walk = ResourcesManager::instance()->get_texture_pool().find(ResID::Tex_SilencerWalk)->second;
		...
		animation_sketch_up.set_frame_data(tex_idle, 6, 4, idx_list_up);
		animation_sketch_down.set_interval(0.12f);
		animation_sketch_down.set_loop(true);
		animation_sketch_down.set_frame_data(tex_idle, 6, 4, idx_list_down);
		animation_sketch_right.set_interval(0.12f);
		animation_sketch_right.set_loop(true);
		animation_sketch_right.set_frame_data(tex_idle, 6, 4, idx_list_right);
		animation_sketch_left.set_interval(0.12f);
		animation_sketch_left.set_loop(true);
		animation_sketch_left.set_frame_data(tex_idle, 6, 4, idx_list_left);
